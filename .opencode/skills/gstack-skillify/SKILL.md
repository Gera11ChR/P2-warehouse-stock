# Skill: gstack-skillify (Knowledge Extraction & Skill Authoring)

## Trigger
Activated manually via `/skillify <workflow-name>` or when a task yields a repeatable technical pattern (e.g., recurring Alembic migration fixes, custom Pydantic bulk validators, or FastAPI setup).

## Execution Workflow
1. **Identify the Pattern:** Extract the exact sequence of terminal commands, code structures, or configuration rules applied during the session.
2. **Determine Scope:** Verify that the workflow does not duplicate existing `openspec-*` commands or sub-agent rules (`@dba-guard`, `@sec-ops`).
3. **Format Skill File:** Draft a standardized markdown skill using the project's standard schema:
   - `Title & Overview`
   - `Trigger Criteria`
   - `Step-by-Step Execution Workflow`
   - `Constraints & Invariants`
4. **Persist Artifact:** Write the new skill file to `.opencode/skills/<kebab-case-name>.md`.
5. **Register Index:** Update `AGENTS.md` under Section 4 (Sub-Agent & Skill Index) to reference the new capability.

## Quality Constraints
- Filenames MUST use strict kebab-case (e.g., `ingest-excel-grid.md`).
- Skills MUST NOT override or contradict rules defined in `docs/constitution.md`.
